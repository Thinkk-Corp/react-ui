...
export const SplitLayout = ({ image, title, subtitle, direction = "normal" }: ISplitLayout) => {
	...
	return (
		...
		>
			{/* Sol taraf - Görsel */}
			<div className={`flex-1 flex h-full items-center p-10 justify-center ${isReversed ? "order-2" : "order-1"}`}>
				<div className={"flex text-center flex-col gap-4 items-center"}>
					{image && <img src={image} alt={title} className="object-cover w-72 h-72 md:w-96 md:h-96" />}
					<h3 className="text-h1 max-w-[30rem] text-color-primary">{title}</h3>
					<p className="w-auto max-w-[30rem] text-wrap break-all text-subtitle2 opacity-70">{subtitle}</p>
				</div>
			</div>

			{/* Sağ taraf - Başlık ve Alt Başlık */}
			<div className={`w-full lg:w-[42rem] h-[42rem] lg:h-full p-10 ${isReversed ? "order-1" : "order-2"}`}>
